...
export function getApproxItemSize(item: BoardItem): ItemSize {
  switch (item.type) {
    ...
    case 'line':
      return { width: Math.abs(item.x2 - item.x), height: Math.abs(item.y2 - item.y) };
    case 'timeline':
    ...
    case 'document':
      return { width: item.width ?? ITEM_WIDTH.document, height: item.height ?? 600 };
    case 'embed':
      return { width: item.width ?? ITEM_WIDTH.embed, height: item.height ?? 320 };
    case 'code':
      return { width: item.width ?? ITEM_WIDTH.code, height: item.height ?? 280 };
    case 'dispenser':
      return { width: item.width ?? ITEM_WIDTH.dispenser, height: item.height ?? 200 };
    case 'note':
      return { width: item.width ?? ITEM_WIDTH.note, height: item.height ?? 170 };
    ...
    case 'section-title':
    case 'text':
      return { width: item.width ?? ITEM_WIDTH.text, height: 60 };

    case 'checklist':
      return { width: item.width ?? ITEM_WIDTH.checklist, height: item.height ?? 200 };

    case 'column':
      return { width: item.width, height: item.height ?? 260 };
...
export function getToolDefaultSize(type: ToolType): ItemSize {
  switch (type) {
    ...
    case 'document':
      return { width: ITEM_WIDTH.document, height: 600 };
    case 'embed':
      return { width: ITEM_WIDTH.embed, height: 320 };
    case 'code':
      return { width: ITEM_WIDTH.code, height: 280 };
    case 'dispenser':
      return { width: ITEM_WIDTH.dispenser, height: 200 };
    case 'divider':
      return { width: ITEM_WIDTH.line, height: 24 };
    case 'note':
      return { width: ITEM_WIDTH.note, height: 160 };
    ...
    case 'section-title':
    case 'text':
      return { width: ITEM_WIDTH.text, height: 64 };

    case 'checklist':
      return { width: ITEM_WIDTH.checklist, height: 192 };

    case 'column':
      return { width: ITEM_WIDTH.column, height: 256 };

    case 'frame':
      return { width: ITEM_WIDTH.frame, height: 256 };
