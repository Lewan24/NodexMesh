...
export function findAlignmentSnap({
  ...
  for (const item of items) {
    ...
    for (const draggedAnchor of draggedXAnchors) {
      for (const targetAnchor of targetXAnchors) {
        ...
        const isCenterMatch =
          draggedAnchor.kind === 'center' &&
          targetAnchor.kind === 'center';
  ...
  const snappedX =
    bestX
      ? bestX.position - bestX.offset
      : undefined;

  const snappedY =
    bestY
      ? bestY.position - bestY.offset
      : undefined;
