...
export function useCanvasClipboard({ projectRef, selectedIdsRef, measuredSizes, pushHistory, onRestoreItems, onSelectItems, pastePoint, nestedSelection, clearColumnSelection }: {
  ...
  const selected = useCallback(() => {
    ...
    if (nestedSelection) {
      ...
      const child = column?.type === 'column' ? column.items.find(item => item.id === nestedSelection.item.id) : undefined;
    ...
    return all.filter(item => ids.has(item.id)).map(item => item.type === 'line' ? resolveLineItem(item, all, measuredSizes) : item);
  ...
  const insert = useCallback((items: BoardItem[], dx: number, dy: number) => {
    if (!items.length) return;
    const all = projectRef.current.items;
