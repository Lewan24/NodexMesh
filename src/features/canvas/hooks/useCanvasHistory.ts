...
export function useCanvasHistory({ getItems, restoreItems, projectId, limit = CANVAS_HISTORY_LIMIT }: UseCanvasHistoryOptions) {
  const state = useRef<{ id: string; history: ItemHistory } | null>(null);
  if (!state.current || state.current.id !== projectId) state.current = { id: projectId, history: new ItemHistory(getItems(), limit) };
  const history = state.current.history;
  // Covers every persisted block mutation, including nested blocks and portal editors.
  useLayoutEffect(() => { history.observe(getItems()); });
  const pushHistory = useCallback(() => { history.observe(getItems()); history.boundary(); }, [history, getItems]);
  ...
  useEffect(() => {
    ...
    let lastKey = 0;
    const boundary = () => { typingTarget = null; pushHistory(); };
    const key = (event: KeyboardEvent) => {
      if ((event.ctrlKey || event.metaKey) && event.key.toLowerCase() === 'z') return;
      const target = event.target;
      const typing = target instanceof HTMLElement && (target.isContentEditable || target.matches('textarea,input:not([type=checkbox]):not([type=range]):not([type=color])'));
      const now = Date.now();
    ...
    return () => { window.removeEventListener('pointerdown', boundary, true); window.removeEventListener('keydown', key, true); };
  }, [pushHistory]);
