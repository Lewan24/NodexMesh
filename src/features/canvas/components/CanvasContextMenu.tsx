...
export default function CanvasContextMenu({ onCopyStyle, onPasteStyle, canPasteStyle, menu, count, canPaste, allLocked, onClose, onCopy, onPaste, onDuplicate, onDelete, onLock, onGroup, onJoinDrawings }: {
  onCopyStyle: () => void; onPasteStyle: () => void; canPasteStyle: boolean;
  onJoinDrawings?: () => void;
  ...
  const actions = [
    ...(menu.hasSelection ? [
      ...
      { name: 'Paste style', keys: '', icon: ClipboardPaste, run: onPasteStyle, disabled: !canPasteStyle || allLocked },
  ...
  return createPortal(<div ref={ref} role="menu" aria-label="Canvas actions" className="canvas-context-menu" style={{ left: position.x, top: position.y }} onMouseDown={event => event.stopPropagation()} onContextMenu={event => event.preventDefault()} onKeyDown={event => {
    ...
    {actions.map(action => <button key={action.name} role="menuitem" disabled={action.disabled} onClick={() => run(action.run)} className={action.name === 'Delete' ? 'text-rose-500' : ''}><action.icon size={15} /><span className="flex-1 text-left">{action.name}</span><kbd>{action.keys}</kbd></button>)}
  </div>, document.body);
}
