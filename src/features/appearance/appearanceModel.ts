...
export function paletteBackground(palette: Palette, key: PaletteKey | 'canvas'): string {
  ...
  return gradient ? gradientBackground(gradient) : palette[key];
}
export interface Appearance {
  light: Palette; dark: Palette; font: FontFamily; mode?: 'light' | 'dark';
}
export interface AppearancePreferences {
  defaults: Appearance; projects: Record<string, Appearance>; uiFont: FontFamily; uiPrimary: string; uiSecondary: string; inheritanceVersion?: number;
}
export const defaultAppearance: Appearance = {
  font: 'short-stack',
  light: { primary: '#6023b4', secondary: '#176f86', canvas: '#f4f1f9', default: '#ffffff', accent1: '#ede4fa', accent2: '#dceff2', accent3: '#e3f1e4', accent4: '#fff0d5', accent5: '#f8e2eb' },
  dark: { primary: '#ab80e5', secondary: '#57bdc8', canvas: '#14101d', default: '#241b30', accent1: '#39264f', accent2: '#163b43', accent3: '#243e2d', accent4: '#463722', accent5: '#482839' },
};
export const preferenceKey = (userId: string) => 'nodexmesh_appearance_' + userId;
export function newPreferences(): AppearancePreferences { return { defaults: structuredClone(defaultAppearance), projects: {}, uiFont: 'sans', uiPrimary: '#6023b4', uiSecondary: '#176f86', inheritanceVersion: 1 }; }
...
export function readPreferences(userId: string): AppearancePreferences {
  try {
    ...
    if (raw?.defaults?.light && raw?.defaults?.dark && raw?.projects) return raw.inheritanceVersion === 1 ? { ...newPreferences(), ...raw } : migratePreferences({ ...newPreferences(), ...raw });
...
export function migratePreferences(preferences: AppearancePreferences): AppearancePreferences {
  ...
  const sameStyle = (a: Appearance, b: Appearance) => a.font === b.font && JSON.stringify(a.light.gradients) === JSON.stringify(b.light.gradients) && JSON.stringify(a.dark.gradients) === JSON.stringify(b.dark.gradients) &&
    (['light', 'dark'] as const).every(mode => Object.keys(b[mode]).every(key => a[mode]?.[key as keyof Palette] === b[mode][key as keyof Palette]));
