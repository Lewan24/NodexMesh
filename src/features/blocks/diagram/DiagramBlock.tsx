...
export default function DiagramBlock({
  ...
  useEffect(() => {
    setNodes((current) =>
      item.nodes.map((node) => ({ ...node, selected: current.find((existing) => existing.id === node.id)?.selected })),
    );
  }, [item.nodes]);
  useEffect(() => {
    setEdges(item.edges);
  ...
  const save = (nextNodes: FlowNode[], nextEdges: Edge[]) => {
    ...
    setEdges(nextEdges);
    onUpdate((current) =>
      current.type === 'diagram' ? { ...current, nodes: toNodes(nextNodes), edges: toEdges(nextEdges) } : current,
    );
  };
    ...
    save(
      [
        ...nodes,
        {
          ...
          type: 'shape',
          position: {
            x: Math.round((120 - viewport.x) / viewport.zoom / 16) * 16,
            y: Math.round((100 - viewport.y) / viewport.zoom / 16) * 16,
          },
          data: { label: shapes.find((option) => option.value === shape)!.label, shape, color: '#7c3aed' },
        },
      ],
      edges,
    );
  ...
  const deleteSelection = () => {
    if (selection.node || nodes.some((node) => node.selected)) {
      const next = removeDiagramNodes(
        toNodes(nodes),
        toEdges(edges),
        new Set(
      ...
      save(
        nodes,
        edges.filter((edge) => edge.id !== selection.edge),
      );
    setSelection({});
  };
  const content = (
    <>
      ...
      {editing && (
        <div className="planning-toolbar" onMouseDown={(event) => event.stopPropagation()}>
          <button
            className="planning-button"
            onClick={() => {
              clickedSelection.current = new Set(nodes.map((node) => node.id));
          ...
          {(['x', 'y'] as const).map((axis) => (
            <button
              ...
              onClick={() =>
                save(
                  alignDiagramNodes(nodes, new Set(nodes.filter((node) => node.selected).map((node) => node.id)), axis),
                  edges,
                )
              }
            >
          ...
          {shapes.map((shape) => (
            <button className="planning-button" key={shape.value} onClick={() => addNode(shape.value)}>
              + {shape.label}
            </button>
          ))}
          <select
      ...
      <div
        ...
        data-wheel-scroll={editing ? 'true' : undefined}
          ...
          <div className="absolute inset-0 flex flex-col items-center justify-center gap-3 pointer-events-none">
            ...
            <button
              ...
              onClick={() => {
                ...
                save(template.nodes, template.edges);
                setEditing(true);
          ...
          {selectedNode ? (
              ...
              <select
                ...
                onChange={(event) =>
                  save(
                    nodes.map((node) =>
                      node.id === selectedNode.id
                        ? { ...node, data: { ...node.data, shape: event.target.value as DiagramShape } }
                        : node,
                    ),
                    edges,
                  )
              ...
              <input
                ...
                onChange={(event) =>
                  save(
                    nodes.map((node) =>
                      node.id === selectedNode.id
                        ? { ...node, data: { ...node.data, color: event.target.value } }
                        : node,
                    ),
                    edges,
                  )
            ...
            <input
              ...
              onChange={(event) =>
                save(
                  nodes,
                  edges.map((edge) => (edge.id === selectedEdge.id ? { ...edge, label: event.target.value } : edge)),
                )
              }
            />
          ) : (
            <input
              ...
              onChange={(event) =>
                onUpdate((current) =>
                  current.type === 'diagram' ? { ...current, title: event.target.value } : current,
                )
              }
            />
          )}
          ...
          <label className="text-sm">
            ...
            <select
              ...
              onChange={(event) =>
                save(
                  nodes,
                  edges.map((edge) => (edge.id === selectedEdge.id ? { ...edge, type: event.target.value } : edge)),
                )
              }
            >
      ...
      {editing && selectedNode && (
        <div
          className="planning-toolbar max-h-28 overflow-auto"
        ...
        >
              ...
              >
                ...
                <button
                  ...
                  onClick={() =>
                    save(
                      nodes,
                      edges.filter((current) => current.id !== edge.id),
                    )
                  }
                >
                  ×
          ...
          <button
            className="planning-button ml-auto"
            disabled={!edges.some((edge) => edge.source === selectedNode.id || edge.target === selectedNode.id)}
            onClick={() =>
              save(
                nodes,
