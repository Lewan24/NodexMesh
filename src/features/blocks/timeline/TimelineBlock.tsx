...
export default function TimelineBlock({ item, onUpdate, onDelete }: { item: TimelineItem; onUpdate: BlockUpdateHandler; onDelete: BlockDeleteHandler }) {
  ...
  const moveBar = (event: React.PointerEvent<HTMLDivElement>, task: TimelineTask, resize: boolean, bar?: HTMLDivElement) => {
    ...
    const finish = (end: PointerEvent) => {
      ...
      if (end.type !== 'pointercancel' && movedBar.current) updateTask(task.id, current => shiftTask(current, Math.round((end.clientX - origin) / (dayWidth * scale)), resize));
    };
  ...
  return <ContentBlockShell item={item} onDelete={onDelete} title={<span className="flex items-center justify-between gap-2"><span className="truncate">{item.title}</span><span className="text-xs opacity-50">{item.tasks.filter(task => task.done).length}/{item.tasks.length} done</span></span>}>
      ...
      {item.mode === 'simple' ? <div className="p-5">{item.tasks.map((task, index) => <article key={task.id} className="timeline-card pr-20">
        ...
        {visibleTasks.map(task => { const dates = taskRange(task); return <div className="contents" key={task.id}>
          <div className="timeline-label flex items-center gap-1 !p-1" style={{ boxShadow: dropRow === task.id ? 'inset 0 2px var(--color-accent)' : undefined }} onMouseDown={event => event.stopPropagation()}
            ...
            <button className="truncate flex-1 text-left py-2" onClick={() => { setDraft({ ...task, checklist: task.checklist.map(entry => ({ ...entry })) }); setEditing(true); }}>{task.done ? '✓ ' : ''}{task.title}</button>
    ...
    {draft && <TimelineTaskDialog task={draft} isNew={!item.tasks.some(task => task.id === draft.id)} onClose={() => setDraft(null)} onSave={task => {
      update(current => ({ ...current, tasks: current.tasks.some(entry => entry.id === task.id) ? current.tasks.map(entry => entry.id === task.id ? task : entry) : [...current.tasks, task] }));
      setDraft(null);
    }} onDelete={() => { update(current => ({ ...current, tasks: current.tasks.filter(task => task.id !== draft.id) })); setDraft(null); }} />}
  </ContentBlockShell>;
}
