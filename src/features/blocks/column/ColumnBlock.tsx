...
import {
  useColumnDrag,
} from '@/features/blocks/column/hooks/useColumnDrag';
...
export default function ColumnBlock({
  ...
  useEffect(() => {
    ...
    const handleOutsideClick =
      (
        ...
        const outsideBackgroundMenu =
          !backgroundMenuRef.current
            ?.contains(target);

        if (
          outsideColumn &&
          outsideBackgroundMenu
        ) {
          setShowAddMenu(
            false,
          );
  ...
  const addNewItem =
    useCallback(
      (
        kind:
          Parameters<
            typeof createDefaultColumnItem
          >[0],
      ) => {
        updateItems(
          currentItems => [
  ...
  function prepareNestedItemForColumn(
    item: BoardItem,
    innerWidth: number,
  ): BoardItem {
    if (item.type === 'note') {
      ...
      {isDragOver && (
        <div
          ...
          style={{
            ...
            boxShadow:
              '0 0 0 3px #7C3AED, 0 0 24px rgba(124, 58, 237,0.3)',
