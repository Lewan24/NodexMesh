  ...
  return (
    <ContentBlockShell
      ...
      title={
        ...
        /> : <span className="flex-1 truncate" onDoubleClick={() => setEditing(true)}>{item.title}</span>}
        <button type="button" className="shrink-0 rounded px-2 py-1 text-xs hover:bg-violet-500/10" aria-pressed={autoHeight} title="Fit content and grow automatically while writing" onMouseDown={e => e.stopPropagation()} onClick={() => onUpdate(current => current.type === 'document' ? { ...current, autoHeight: true, height: undefined } : current)}>Auto-fit</button>
        <button type="button" className="shrink-0 rounded px-2 py-1 text-xs hover:bg-violet-500/10" onMouseDown={e => e.stopPropagation()} onClick={() => setEditing(!editing)}>{editing ? 'Done' : 'Edit'}</button>
        </div>
      }
    >
      {editing && (
      ...
      >
        <EditorContent editor={editor} className={`document-editor ${editing ? '' : 'pointer-events-none select-none'}`} />
      </div>
    </ContentBlockShell>
  )
}
