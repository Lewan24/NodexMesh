...
export default function SectionTitleBlock({
  ...
  return (
    <div style={{ width: item.width ?? 320, height: 36, position: 'relative' }}>
      <div
        className="absolute pointer-events-auto"
        style={{
          left: 0,
          top: 36,
          transform: `translateY(-100%) scale(${sectionTitleScale(zoom)})`,
      ...
      >
        <SectionLabel
          ...
          zoom={zoom}
          onChange={(content) =>
            onUpdate((current) => (current.type === 'section-title' ? { ...current, content } : current))
          }
        />
      </div>
    </div>
