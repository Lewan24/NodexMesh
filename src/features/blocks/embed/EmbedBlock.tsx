...
export default function EmbedBlock({
  ...
  const update = (patch: Partial<EmbedItem>) =>
    onUpdate((current) => (current.type === 'embed' ? { ...current, ...patch } : current));
  if (videoId && !editing)
    return (
      <section
        className="group/video relative item-rounded shadow-xl flex flex-col"
        style={{ width: item.width, height: item.height, background, color: textColor, ...getTypographyStyle(item) }}
      >
    ...
    >
      {editing ? (
        <form
          ...
          onMouseDown={(e) => {
            if ((e.target as HTMLElement).closest('input, button, label')) e.stopPropagation();
          }}
          onSubmit={(e) => {
            e.preventDefault();
