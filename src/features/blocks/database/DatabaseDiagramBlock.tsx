...
import {
  ...
  createDatabaseTable,
  databaseExample,
  validDatabaseRelations,
  canAddDatabaseRelation,
} from './databaseUtils';
...
export default function DatabaseDiagramBlock({
  item,
  onUpdate,
  onDelete,
}: {
  item: DatabaseDiagramItem;
  ...
  const makeNodes = (tables: DatabaseTable[], relations: DatabaseRelation[]): TableNode[] =>
    tables.map((table) => ({
      id: table.id,
      type: 'table',
  ...
  const save = (tables: DatabaseTable[], relations = item.relations) =>
    onUpdate((current) =>
      current.type === 'database'
        ? { ...current, tables, relations: validDatabaseRelations(tables, relations) }
        : current,
    );
  const table = item.tables.find((entry) => entry.id === selected);
  const relation = item.relations.find((entry) => entry.id === relationId);
  const updateTable = (patch: Partial<DatabaseTable>) =>
    save(item.tables.map((entry) => (entry.id === selected ? { ...entry, ...patch } : entry)));
  const edges = useMemo(
    () =>
      item.relations.map((relation) => ({
        ...
        sourceHandle:
          ...
          ((item.tables.find((table) => table.id === relation.source)?.position.x ?? 0) <
          (item.tables.find((table) => table.id === relation.target)?.position.x ?? 0)
            ? ':right'
            : ':left'),
        targetHandle:
        ...
        interactionWidth: 24,
        selected: relation.id === relationId,
      })),
    [item.relations, item.tables, relationId],
  );
  const fit = () => requestAnimationFrame(() => flow.current?.fitView({ padding: 0.2, duration: 200 }));
  const content = (
    <div className="flex flex-1 min-h-0">
      <div className="flex flex-col flex-1 min-w-0">
          ...
          {editing ? (
            <>
              <button
                ...
                onClick={() => {
                  const next = createDatabaseTable('table_' + (item.tables.length + 1), item.tables.length);
                  save([...item.tables, next]);
                  setSelected(next.id);
                  setRelationId(null);
                  fit();
                }}
                  ...
                  save(
                    item.tables.map((table, index) => ({
                      ...
                      position: {
                        ...
                        y:
                          Math.floor(index / 3) *
                          (Math.max(3, ...item.tables.map((table) => table.fields.length)) * 36 + 112),
                      },
                    })),
                  );
                  fit();
        ...
        >
          <div className="absolute inset-0" style={{ pointerEvents: editing ? 'auto' : 'none' }}>
            {!editing ? (
              <DatabasePreview tables={item.tables} relations={item.relations} />
            ) : (
              <ReactFlow<TableNode>
                ...
                onNodeDragStop={(_, node, dragged) =>
                  save(
                    item.tables.map((table) => {
                      const moved =
                        dragged.find((entry) => entry.id === table.id) ?? (table.id === node.id ? node : undefined);
                      return moved ? { ...table, position: moved.position } : table;
                    }),
                  )
                }
                onNodeClick={(_, node) => {
                ...
                onConnect={(connection) => {
                  const relation: DatabaseRelation = {
                    ...
                    targetField: connection.targetHandle?.replace(/:(left|right)$/, '') ?? '',
                    cardinality: 'N:1',
                  };
                  if (canAddDatabaseRelation(item.tables, item.relations, relation)) {
                    save(item.tables, [...item.relations, relation]);
          ...
          {!item.tables.length && (
            <div className="absolute inset-0 flex items-center justify-center pointer-events-none">
              <button
                ...
                onClick={() => {
                  const example = databaseExample();
                  save(example.tables, example.relations);
                  setEditing(true);
                  fit();
                }}
              >
      ...
      {editing && (
        <aside
          className="w-80 shrink-0 p-4 space-y-3 overflow-auto border-l border-current/10"
        ...
        >
          <label className="block text-sm">
            ...
            <input
              ...
              onChange={(event) =>
                onUpdate((current) =>
                  current.type === 'database' ? { ...current, title: event.target.value } : current,
                )
              }
            />
          </label>
          {table ? (
            <>
              ...
              {table.fields.map((field, index) => (
                <fieldset key={field.id} className="p-2 bg-violet-500/5 space-y-2 rounded-sm">
                  ...
                  <input
                    ...
                    onChange={(event) =>
                      updateTable({
                        fields: table.fields.map((entry) =>
                          entry.id === field.id ? { ...entry, name: event.target.value } : entry,
                        ),
                      })
                    }
                  />
                  <input
                    ...
                    onChange={(event) =>
                      updateTable({
                        fields: table.fields.map((entry) =>
                          entry.id === field.id ? { ...entry, dataType: event.target.value } : entry,
                        ),
                      })
                    }
                  />
                  <div className="flex flex-wrap gap-2 text-xs">
                    {(['primaryKey', 'nullable', 'unique'] as const).map((key) => (
                      <label key={key}>
                        <input
                          type="checkbox"
                          checked={field[key]}
                          disabled={key === 'nullable' && field.primaryKey}
                          onChange={(event) =>
                            updateTable({
                              fields: table.fields.map((entry) =>
                                entry.id === field.id
                                  ? {
                                      ...
                                      ...(key === 'primaryKey' && event.target.checked ? { nullable: false } : {}),
                                    }
                                  : entry,
                              ),
                            })
              ...
              <button
                className="planning-button text-rose-500"
                onClick={() => {
                  save(item.tables.filter((entry) => entry.id !== table.id));
                  setSelected(null);
                }}
              >
                Delete table
            ...
            <>
              ...
              <p className="text-xs">
                ...
                {
                  item.tables
                    .find((table) => table.id === relation.source)
                    ?.fields.find((field) => field.id === relation.sourceField)?.name
                }{' '}
                → {item.tables.find((table) => table.id === relation.target)?.name}.
                {
                  item.tables
                    .find((table) => table.id === relation.target)
                    ?.fields.find((field) => field.id === relation.targetField)?.name
                }
              </p>
              <label className="block text-sm">
                Cardinality
                    ...
                    save(
                      ...
                      item.relations.map((entry) =>
                        entry.id === relation.id
                          ? { ...entry, cardinality: event.target.value as DatabaseRelation['cardinality'] }
                          : entry,
                      ),
                    )
                  }
                >
                  {(['1:1', '1:N', 'N:1', 'N:N'] as const).map((value) => (
              ...
              <button
                ...
                onClick={() => {
                  save(
                    item.tables,
                    item.relations.filter((entry) => entry.id !== relation.id),
                  );
                  setRelationId(null);
                }}
              >
