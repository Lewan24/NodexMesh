...
export function validDatabaseRelations(tables: DatabaseTable[], relations: DatabaseRelation[]): DatabaseRelation[] {
  return relations.filter((relation) => {
    ...
    return (
      source?.fields.some((field) => field.id === relation.sourceField) &&
      target?.fields.some((field) => field.id === relation.targetField) &&
      !(relation.source === relation.target && relation.sourceField === relation.targetField)
    );
  });
}
export function canAddDatabaseRelation(
  tables: DatabaseTable[],
  relations: DatabaseRelation[],
  relation: DatabaseRelation,
): boolean {
  return (
    validDatabaseRelations(tables, [relation]).length === 1 &&
    !relations.some(
      (existing) =>
        existing.source === relation.source &&
        existing.target === relation.target &&
        existing.sourceField === relation.sourceField &&
        existing.targetField === relation.targetField,
    )
  );
}
