...
export default function DrawingBlock({ item }: { item: DrawingItem }) {
  return (
    ...
    >
      {drawingStrokes(item).map((stroke, index) => (
        <path
          key={index}
          transform={`translate(${stroke.x} ${stroke.y}) scale(${stroke.scaleX} ${stroke.scaleY})`}
          d={drawingOutline(stroke.points, stroke.strokeWidth)}
          fill={stroke.color}
        />
      ))}
    </svg>
