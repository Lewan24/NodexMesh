...
export function smoothDrawing(points: DrawingPoint[]): DrawingPoint[] {
  ...
  for (let index = 1; index < points.length; index++) {
    ...
    const length =
      Math.hypot(control.x - start.x, control.y - start.y) + Math.hypot(end.x - control.x, end.y - control.y);
...
export function drawingOutline(points: DrawingPoint[], strokeWidth: number): string {
  const samples = smoothDrawing(points).filter(
    (point, index, all) => index === 0 || Math.hypot(point.x - all[index - 1]!.x, point.y - all[index - 1]!.y) > 0.001,
  );
  if (!samples.length) return '';
  const radius = (point: DrawingPoint) => Math.max(0.1, (strokeWidth * (point.pressure ?? 1)) / 2);
  ...
  samples.forEach((point, index) => {
    const before = samples[Math.max(0, index - 1)]!,
      after = samples[Math.min(samples.length - 1, index + 1)]!;
    const angle = Math.atan2(after.y - before.y, after.x - before.x);
    const nx = -Math.sin(angle) * radius(point),
      ny = Math.cos(angle) * radius(point);
...
export function drawingStrokes(item: DrawingItem): DrawingStroke[] {
  return (
    item.strokes ?? [
      { points: item.points, x: 0, y: 0, scaleX: 1, scaleY: 1, color: item.color, strokeWidth: item.strokeWidth },
    ]
  );
}
...
export function joinDrawings(items: DrawingItem[]): DrawingItem | null {
  ...
  return {
    ...
    strokes: ordered.flatMap((item) => {
      const sx = item.width / item.viewWidth,
        sy = item.height / item.viewHeight;
      return drawingStrokes(item).map((stroke) => ({
        ...stroke,
        points: stroke.points.map((point) => ({ ...point })),
        x: item.x - x + stroke.x * sx,
        y: item.y - y + stroke.y * sy,
        scaleX: stroke.scaleX * sx,
        scaleY: stroke.scaleY * sy,
      }));
    }),
  };
}
