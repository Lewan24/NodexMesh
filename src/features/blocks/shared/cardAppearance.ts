...
export function resolveAppearance(color: string | undefined, palette: Palette, gradient?: BaseItem['gradient'], role?: BaseItem['colorRole']) {
  ...
  const background = gradient ? gradient.kind === 'radial'
    ? `radial-gradient(circle at center, ${from}, ${to})`
    : `linear-gradient(${gradient.angle}deg, ${from}, ${to})` : solid;
  const light = isLightColor(from) && isLightColor(to);
  return { background, solid, light, textColor: light ? '#172033' : '#f8fafc', mutedColor: light ? '#475569' : '#c8bed5' };
}
