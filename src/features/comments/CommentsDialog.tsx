...
import type { BoardItem, CommentStatus, ItemComment } from '@/entities/board/types';

import { createComment } from '@/features/comments/utils/commentUtils';
...
export default function CommentsDialog({ item, onUpdate, onClose }: CommentsDialogProps) {
    ...
    onUpdate((current) => ({
      ...
      comments:
        current.comments?.map((comment) =>
          comment.id === id ? { ...comment, ...patch, updatedAt: new Date().toISOString() } : comment,
        ) ?? [],
    }));
  };
  ...
  return (
    <div
      ...
      style={{ backgroundColor: 'rgba(8,16,20,0.55)', backdropFilter: 'blur(3px)' }}
      onMouseDown={(event) => {
