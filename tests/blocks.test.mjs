...
const { getArrowHeadPoints, getLineCurve } = await server.ssrLoadModule(
  '/src/features/blocks/line/utils/lineRenderGeometry.ts',
);
const { isFrameMovementLocked, normalizeFrameMembership, getFrameContents } = await server.ssrLoadModule(
  '/src/features/canvas/utils/frameGeometry.ts',
);
const { ItemHistory } = await server.ssrLoadModule('/src/features/canvas/utils/itemHistory.ts');
const { resolveCardColor, resolveAppearance } = await server.ssrLoadModule(
  '/src/features/blocks/shared/cardAppearance.ts',
);
const { autoGrowthLayout, growsAutomatically } = await server.ssrLoadModule(
  '/src/features/canvas/utils/autoGrowthLayout.ts',
);
const { ITEM_WIDTH, CANVAS_GRID_SIZE } = await server.ssrLoadModule('/src/features/canvas/constants.ts');
const { createDatabaseField, databaseExample, validDatabaseRelations, canAddDatabaseRelation } =
  await server.ssrLoadModule('/src/features/blocks/database/databaseUtils.ts');
const { defaultAppearance, activeAppearance, newPreferences, preferenceKey, withAppearanceMode, migratePreferences } =
  await server.ssrLoadModule('/src/features/appearance/appearanceModel.ts');
const { copyItemStyle, pasteItemStyle } = await server.ssrLoadModule('/src/features/canvas/utils/itemStyle.ts');
const { COLUMN_ADD_TYPES, createDefaultColumnItem } = await server.ssrLoadModule(
  '/src/features/blocks/column/utils/columnItems.ts',
);
...
test('divider snaps both endpoints and cannot resolve item attachments', () => {
  ...
  for (const key of ['x', 'y', 'x2', 'y2']) assert.equal(Math.abs(divider[key] % 16), 0);
  assert.equal(divider.arrowEnd, false);
  const note = createCanvasItem('note', 200, 200);
  const resolved = resolveLineItem({ ...divider, startItemId: note.id, endItemId: note.id, arrowEnd: true }, [note]);
...
test('kanban copies retain independent empty columns; media copies clear sources', () => {
  ...
  for (const type of ['image', 'embed', 'link']) {
    const sibling = createEmptySibling({ ...createCanvasItem(type, 0, 0), url: 'https://example.com' });
...
test('dispenser creates centered flashcards without forcing a placement lattice', () => {
  const first = createCanvasItem('note', 16, 32, { color: '#fde68a', dispenserId: 'stack' });
...
test('embed URL conversion isolates videos and rejects executable URLs', () => {
  for (const value of [
    ...
    assert.equal(getEmbedUrl(value), 'https://www.youtube-nocookie.com/embed/dQw4w9WgXcQ');
  }
  assert.equal(getEmbedUrl('https://vimeo.com/123456'), 'https://player.vimeo.com/video/123456');
  assert.equal(getEmbedUrl('https://example.com/app'), 'https://example.com/app');
  for (const value of [
    'javascript:alert(1)',
...
test('new block content participates in board search', () => {
  ...
  assert.match(getSearchableText({ ...createCanvasItem('code', 0, 0), content: 'const answer = 42;' }), /answer/);
  assert.match(getSearchableText({ ...createCanvasItem('embed', 0, 0), url: 'https://example.com' }), /example.com/);
});
...
test('timeline dates reject invalid days and remain stable across DST and year boundaries', () => {
  ...
  assert.deepEqual(shiftTask({ start: '2026-12-31', end: '2026-12-31' }, 1), {
    start: '2027-01-01',
    end: '2027-01-01',
  });
    ...
    tasks: [
      {
        ...
        color: '#7c3aed',
        checklist: [{ id: 'c', text: 'Deploy API', done: false }],
      },
    ],
  };
...
test('duplicating nested content retains local positions and remaps graph and checklist IDs', () => {
  ...
  const column = {
    ...createCanvasItem('column', 100, 200),
    items: [graph, { ...createCanvasItem('checklist', 0, 0), entries: [{ id: 'check', text: 'Ship', done: true }] }],
  };
...
test('pen speed produces bounded gradual width and pressure survives copying', () => {
  ...
  assert.ok(penPressure(1000, 0) >= 0.45);
  assert.ok(penPressure(0, 100) <= 1.6);
  const drawing = createDrawing(
    [
      { x: 0, y: 0, pressure: 1.5 },
  ...
  assert.deepEqual(
    copy.points.map((point) => point.pressure),
    [1.5, 0.5],
  );
...
test('filled arrowhead geometry scales with line thickness in both directions', () => {
  for (const angle of [0, Math.PI, Math.PI / 3]) {
    ...
    assert.equal(thick.tipX, 50);
    assert.equal(thick.tipY, 50);
  }
});
...
test('joined drawings preserve separate ink, pressure, styles and resized geometry', () => {
  ...
  for (const [index, source] of [b, a].entries()) {
    ...
    source.points.forEach((point, i) => {
      assert.equal(
        joined.x + stroke.x + stroke.points[i].x * stroke.scaleX,
        source.x + (point.x * source.width) / source.viewWidth,
      );
      assert.equal(
        joined.y + stroke.y + stroke.points[i].y * stroke.scaleY,
        source.y + (point.y * source.height) / source.viewHeight,
      );
    });
  }
...
test('history groups gestures, ignores no-ops and isolates project resets', () => {
  ...
  history.boundary();
  history.observe(structuredClone(original));
  history.boundary();
  let moved;
  for (let x = 1; x < 20; x++) {
    moved = [{ ...original[0], x }];
...
test('default white cards follow the theme; custom colors remain fixed', () => {
  ...
  for (const type of [
    ...
    assert.equal(createCanvasItem(type, 0, 0).color, '#ffffff');
});
...
test('native text undo does not leave an empty board undo step', () => {
  ...
  const moved = [{ ...initial[0], x: 100 }];
  history.observe(moved);
  history.boundary();
  const typed = [{ ...moved[0], content: 'Text' }];
  history.observe(typed);
  history.observe(moved);
...
test('automatic growth pushes a vertical chain while preserving other columns and overlaps', () => {
  ...
  const before = new Map([
    ...
    [first.id, { width: 320, height: 80 }],
    [second.id, { width: 320, height: 80 }],
  ]);
...
test('dropping a task on canvas creates an independent checklist with its completion', () => {
  ...
  assert.equal(checklist.x, -32);
  assert.equal(checklist.y, 160);
...
test('overlapping frames preserve ownership through movement, transfer, deletion and copying', () => {
  ...
  history.observe(migrated);
  history.boundary();
  history.observe([a, b, transferred]);
...
test('curved lines preserve endpoint tangents and midpoint under translation and reverse bend', () => {
  ...
  assert.equal(straight.centerY, 0);
  assert.equal(straight.startAngle, 0);
  const curve = getLineCurve(0, 0, 200, 0, 0.5);
  assert.equal(curve.centerX, 100);
  assert.equal(curve.centerY, 50);
  assert.ok(curve.startAngle > 0);
  assert.ok(curve.endAngle < 0);
  assert.equal(getLineCurve(0, 0, 200, 0, -0.5).centerY, -50);
  const moved = getLineCurve(-32, 40, 168, 40, 0.5);
  assert.equal(moved.centerX, curve.centerX - 32);
  assert.equal(moved.centerY, curve.centerY + 40);
...
test('database blocks persist, search, clone field references and create empty siblings', () => {
  ...
  assert.deepEqual(sibling.tables, []);
  assert.deepEqual(sibling.relations, []);
});
...
test('database field/table removal cleans relations and undo restores entire schema', () => {
  ...
  const tables = initial.tables.map((table, index) =>
    index === 0 ? { ...table, fields: table.fields.slice(1) } : table,
  );
...
test('project appearance falls back to user defaults and isolates user/project preferences', () => {
  const userA = newPreferences(),
    userB = newPreferences();
...
test('semantic card colors and gradient stops follow palettes while fixed colors persist', () => {
  const light = defaultAppearance.light,
  ...
  assert.equal(
    ...
    `linear-gradient(90deg, ${dark.accent1}, #123456)`,
  );
  assert.ok(
    resolveAppearance(undefined, light, { ...gradient, kind: 'radial' }).background.startsWith(
...
test('theme gradients apply to semantic cards, preserve custom fills and survive persistence', () => {
  ...
  assert.equal(
    resolveAppearance(undefined, palette, undefined, 'accent1').background,
    'linear-gradient(90deg, #eeeeff, #ffffff)',
  );
  ...
  assert.equal(
    resolveAppearance(undefined, palette, { from: '#000000', to: '#111111', angle: 45, kind: 'linear' }, 'accent1')
      .background,
...
test('style copying applies to a selection without copying content, dimensions or identity', async () => {
  const source = {
    ...
    color: '#123456',
    colorRole: 'accent1',
    typography: { fontSize: 24, bold: true },
    content: 'private',
    x: 10,
    width: 300,
  };
  ...
  assert.equal(pasted.id, 'target');
  assert.equal(pasted.x, 50);
  assert.equal(pasted.width, 400);
  assert.equal(pasted.content, undefined);
  assert.equal(pasted.gradient, undefined);
  assert.equal(pasted.typography.fontSize, 24);
  pasted.typography.fontSize = 16;
  assert.equal(style.typography.fontSize, 24);
  const locked = { ...target, locked: true };
  assert.equal(pasteItemStyle(locked, style), locked);
});
...
test('timeline window shows overlapping tasks and restores hidden tasks without deleting data', async () => {
  const tasks = [
    ...
    { id: 'b', start: '2026-02-01', end: '2026-03-01' },
    { id: 'c', start: '', end: '' },
  ];
  assert.deepEqual(
    tasksInWindow(tasks, dateDay('2026-01-02'), dateDay('2026-01-04')).map((t) => t.id),
    ['a', 'c'],
  );
  assert.deepEqual(
    tasksInWindow(tasks, dateDay('2026-02-10'), dateDay('2026-02-20')).map((t) => t.id),
    ['b', 'c'],
  );
...
test('column creation and drop support agree for compact content blocks', () => {
  ...
  for (const kind of ['timeline', 'diagram', 'database', 'kanban', 'column', 'frame'])
    assert.equal(DROPPABLE_ON_COLUMN.has(kind), false);
