  ...
  if (item.type === 'kanban')
    return {
      ...
      columns: item.columns.map((column) =>
        column.id === columnId ? { ...column, cards: insert(column.cards) } : column,
      ),
    };
  return item;
}
...
export function useCrossItemDrop({
  ...
  const dropTask = useCallback(
    (sourceId: string, task: ChecklistEntry, clientX: number, clientY: number): boolean => {
      ...
      if (!container || !targetId) {
        ...
        if (
          ...
          !canvas.contains(hit) ||
          hit.closest(
            '[data-board-item-id], [data-edit-bar], [data-item-inspector], [role="dialog"], [role="menu"], button, input, textarea, select',
          )
        )
          return false;
      ...
      const column =
        target.type === 'kanban'
          ? (target.columns.find((column) => column.id === columnElement?.dataset.kanbanColumnId) ?? target.columns[0])
          : undefined;
      ...
      const rowsRoot =
        target.type === 'checklist'
          ? container
          : (columnElement ??
            Array.from(container.querySelectorAll<HTMLElement>('[data-kanban-column-id]')).find(
              (el) => el.dataset.kanbanColumnId === column?.id,
            ));
      const rows = Array.from(
        rowsRoot?.querySelectorAll<HTMLElement>(
          target.type === 'checklist' ? '[data-checklist-entry-index]' : '[data-kanban-card-id]',
        ) ?? [],
      );
      ...
      pushHistory();
      onUpdateItem(root.id, (current) =>
        updateNested(current, targetId, (item) => insertTask(item, task, index, column?.id)),
      );
      return true;
    },
