...
export function getLineRenderGeometry(startX: number, startY: number, endX: number, endY: number): LineRenderGeometry {
  ...
  const svgLeft = dx >= 0 ? -LINE_PADDING : -(LINE_PADDING + Math.abs(dx));

  const svgTop = dy >= 0 ? -LINE_PADDING : -(LINE_PADDING + Math.abs(dy));
...
export function getArrowHeadPoints(x: number, y: number, angle: number, strokeWidth = 2): ArrowHeadPoints {
  ...
  return { firstX, firstY, tipX: x, tipY: y, secondX, secondY };
}
