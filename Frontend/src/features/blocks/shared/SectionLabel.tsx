...
export function sectionTitleScale(zoom: number): number {
  return zoom >= 1 ? 1 : Math.min(3.2, 1 / Math.max(0.1, zoom));
}
  ...
  return (
    <div
      ...
      style={{
        ...
        backgroundColor: labelMode === 'far' ? color : 'var(--color-surface-translucent)',
        border: labelMode === 'far' ? 'none' : `1px solid ${color}66`,
        boxShadow: labelMode === 'far' ? '0 4px 14px rgba(0,0,0,0.18)' : '0 2px 8px rgba(0,0,0,0.08)',
        backdropFilter: labelMode === 'far' ? undefined : 'blur(8px)',
    ...
    >
      <div
        ...
        style={{
          ...
          backgroundColor: labelMode === 'far' ? '#fff' : color,
        }}
      />
