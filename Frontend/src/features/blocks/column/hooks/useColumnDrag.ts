...
import type { BoardItem, ColumnLayout } from '@/entities/board/types';
...
export function useColumnDrag({
  ...
  const handleDragStart = useCallback(
    (fromIndex: number, event: React.MouseEvent) => {
      ...
      const getLinearDropIndex = (clientX: number, clientY: number) => {
        ...
        for (let index = 0; index < itemsRef.current.length; index++) {
          ...
          const center = layout === 'horizontal' ? rect.left + rect.width / 2 : rect.top + rect.height / 2;
      ...
      const getGridDropIndex = (clientX: number, clientY: number) => {
        ...
        return Math.min(closestIndex + (afterCenter ? 1 : 0), itemsRef.current.length);
      };
      ...
      const isOutside = (clientX: number, clientY: number) => {
        ...
        return (
          clientX < rect.left - 40 || clientX > rect.right + 40 || clientY < rect.top - 40 || clientY > rect.bottom + 40
        );
      };
      ...
      const handleMove = (moveEvent: MouseEvent) => {
        ...
        if (isOutside(moveEvent.clientX, moveEvent.clientY)) {
        ...
        const targetIndex = getDropIndex(moveEvent.clientX, moveEvent.clientY);
      ...
      const handleUp = (upEvent: MouseEvent) => {
        ...
        if (isOutside(upEvent.clientX, upEvent.clientY) && onEjectItem) {
          deleteNested(draggedItem.id);

          onEjectItem(draggedItem, upEvent.clientX, upEvent.clientY);
        } else {
          ...
          if (finalDropIndex !== null && finalDropIndex !== fromIndex) {
            updateItems((currentItems) => {
              ...
              const sourceIndex = next.findIndex((candidate) => candidate.id === draggedItem.id);
    ...
    [columnId, containerRef, itemRefsMap, layout, updateItems, deleteNested, onEjectItem],
  );
