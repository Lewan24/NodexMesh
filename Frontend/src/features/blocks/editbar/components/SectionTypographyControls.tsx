...
export default function SectionTypographyControls({
  ...
  return (
    ...
    >
      <span className="shrink-0 font-medium" title={translate('Hold Ctrl or Command to select multiple sections')}>
        {translate('Text sections')}
      </span>
      <select
        multiple
        aria-label={translate('Text sections to style (select one or more)')}
      ...
      <label className="flex items-center gap-1 shrink-0">
        ...
        <input
          ...
          min={MIN_FONT_SIZE}
          max={MAX_FONT_SIZE}
          placeholder={translate('Auto / mixed')}
      ...
      <button type="button" className="shrink-0 underline" onClick={() => update({ color: undefined })}>
        {translate('Auto color')}
      </button>
      <button
        type="button"
        aria-pressed={common('bold') === true}
        aria-label={translate('Bold section text')}
      ...
      <button
        ...
        aria-label={translate('Italic section text')}
      ...
      <select
        aria-label={translate('Section text alignment')}
      ...
      >
        <option value="">{translate('Auto / mixed alignment')}</option>
      ...
      {(item.type === 'note' || item.type === 'text') && (
        <select
          aria-label={translate('Vertical text alignment')}
      ...
      <span className="shrink-0" aria-live="polite">
        {common('color') ?? translate('Auto / mixed color')}
      </span>
      <button type="button" className="shrink-0 underline" onClick={() => update()}>
        {translate('Reset sections')}
      </button>
    </div>
  );
}
