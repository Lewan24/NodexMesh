...
export default function AddUserForm() {
  ...
  return (
    ...
    >
      ...
      <div className="flex items-center gap-2">
        {roles.map((itemRole) => (
          <button
            ...
            style={{
              backgroundColor: role === itemRole ? 'var(--color-accent-soft)' : 'var(--color-surface-alt)',
              color: role === itemRole ? 'var(--color-accent)' : 'var(--color-text-secondary)',
            }}
          >
            {itemRole}
          </button>
      ...
      {error && (
        <p className="text-xs" style={{ color: 'var(--color-danger-strong)' }}>
          {error}
        </p>
      )}
